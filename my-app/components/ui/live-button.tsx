...
//       {/* Animated dot */}
//       <span
//         className={`relative z-10 h-3 w-3 rounded-full bg-cyan-400 transition-all duration-500 ease-out ${
//           isHovered ? "scale-125 bg-cyan-300 shadow-lg shadow-cyan-300/50" : ""
//         } ${
//           isPressed ? "scale-90" : ""
//         } before:absolute before:inset-0 before:animate-pulse before:rounded-full before:bg-cyan-200 before:opacity-0 group-hover:before:opacity-50`}
//       >
//         {/* Ripple effect */}
//         <div
...
import React, { useState } from "react";
import { cn } from "@/lib/utils"; // Optional: makes class merging safer if you have it

// 1. Extend standard button props so it accepts onClick, type, etc.
interface LiveButtonProps
  extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  children: React.ReactNode; // Accept icon + text
  className?: string;
}

// 2. Use a named export to match your "import { LiveButton }..."
export function LiveButton({
  children,
  className = "",
  ...props // Capture onClick, disabled, etc.
}: LiveButtonProps) {
  const [isHovered, setIsHovered] = useState(false);
  const [isPressed, setIsPressed] = useState(false);
  ...
  return (
    <button
      className={cn(
        "group relative flex h-10 min-w-[9.3rem] items-center justify-center gap-2 overflow-hidden rounded-lg border border-gray-300 bg-white px-4 transition-all duration-500 ease-out",
        "before:absolute before:inset-0 before:translate-x-[-100%] before:bg-gradient-to-r before:from-transparent before:via-black/5 before:to-transparent before:transition-transform before:duration-700",
        "hover:scale-105 hover:border-gray-400 hover:shadow-lg hover:shadow-black/20 hover:before:translate-x-[100%]",
        "active:scale-95",
      ...
      onMouseDown={() => setIsPressed(true)}
      onMouseUp={() => setIsPressed(false)}
      {...props} // 3. Spread props here so onClick works
    >
      {/* Subtle glow effect */}
      <div className="absolute inset-0 rounded-lg bg-gradient-to-r from-cyan-200/0 via-cyan-200/10 to-cyan-200/0 opacity-0 transition-opacity duration-500 group-hover:opacity-100 dark:from-cyan-200/0 dark:via-cyan-200/10 dark:to-cyan-200/0"></div>

      {/* Content (Icon + Text) */}
      <span className="relative z-10 flex items-center gap-2 text-sm font-medium tracking-wide whitespace-nowrap text-black transition-all duration-300 group-hover:text-cyan-600 dark:text-white dark:group-hover:text-cyan-50">
        {children}
      </span>

      {/* Animated dot */}
      <span
        className={`relative z-10 h-2 w-2 rounded-full bg-cyan-400 transition-all duration-500 ease-out ${
          isHovered ? "scale-125 bg-cyan-300 shadow-lg shadow-cyan-300/50" : ""
        } ${
          isPressed ? "scale-90" : ""
        } before:absolute before:inset-0 before:animate-pulse before:rounded-full before:bg-cyan-200 before:opacity-0 group-hover:before:opacity-50`}
